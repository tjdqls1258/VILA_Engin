#ifndef GL_OBJLOADER_H_
#define GL_OBJLOADER_H_
...
static std::vector<Vertex> loadOBJ(const char* file_name)
{
	...
	{
		if (!vertex_position_indicies.empty() && !vertex_positions.empty())
		{
			vertices[i].position = vertex_positions[vertex_position_indicies[i] - 1];
		}
		if (!vertex_texcoord_indicies.empty() && !vertex_texcoords.empty())
		{
			vertices[i].texcoord = vertex_texcoords[vertex_texcoord_indicies[i] - 1];
		}
		if (!vertex_normal_indicies.empty() && !vertex_normals.empty())
		{
